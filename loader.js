...
var Loader = function (options) {
  this.options = {
    ...
    assetsjsondir: options.assetsjsondir
  }
...
Loader.prototype.dev = function () {
  const {viewdir, assetsdir, assetsjsondir} = this.options;
  // console.log(this.options)
  delDir(assetsdir);
  runBuilder(viewdir, assetsjsondir, true)
  const CDNMap = JSON.parse(fs.readFileSync(path.join(assetsjsondir, './assets.json'), 'utf-8'));
  var html = '';
  var scriptTarget = this.script.target;
  if (scriptTarget && CDNMap[scriptTarget]) {
    html += '<script src="' + CDNMap[scriptTarget] + '"></script>\n';
  }
  var styleTarget = this.style.target;
  if (styleTarget && CDNMap[styleTarget]) {
    html += '<link rel="stylesheet" href="' + CDNMap[styleTarget] + '" media="all" />\n';
  }
